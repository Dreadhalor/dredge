// bait bucket
export { default as MixedBaitImage } from './mixed-bait.webp';
// dog tags
export { default as DogTagsImage } from './dog-tags.webp';
// bolt of cloth
export { default as BoltOfClothImage } from './bolt-of-cloth.webp';
// metal scraps
export { default as MetalScrapsImage } from './metal-scraps.webp';
// lumber
export { default as LumberImage } from './lumber.webp';
// packed explosives
export { default as PackedExplosivesImage } from './packed-explosives.webp';
// research part
export { default as ResearchPartImage } from './research-part.webp';
// refined metal
export { default as RefinedMetalImage } from './refined-metal.webp';
// chunk of flesh
export { default as ChunkOfFleshImage } from './chunk-of-flesh.webp';
// broken spectacles
export { default as BrokenSpectaclesImage } from './broken-spectacles.webp';
// doubloon
export { default as DoubloonImage } from './doubloon.webp';
// bag of doubloons
export { default as BagOfDoubloonsImage } from './bag-of-doubloons.webp';
// big bag of doubloons
export { default as BigBagOfDoubloonsImage } from './big-bag-of-doubloons.webp';
// sextant
export { default as SextantImage } from './sextant.webp';
// belt buckle
export { default as BeltBuckleImage } from './belt-buckle.webp';
// family crest
export { default as FamilyCrestImage } from './family-crest.webp';
// fathomless flame
export { default as FathomlessFlameImage } from './fathomless-flame.webp';
// fused tablet
export { default as FusedTabletImage } from './fused-tablet.webp';
// mortar barrel
export { default as MortarBarrelImage } from './mortar-barrel.webp';
// mortar frame
export { default as MortarFrameImage } from './mortar-frame.webp';
// old iron chain
export { default as OldIronChainImage } from './old-iron-chain.webp';
// aurous anchor
export { default as AurousAnchorImage } from './aurous-anchor.webp';
// broken monocle
export { default as BrokenMonocleImage } from './broken-monocle.webp';
// worn gold ring
export { default as WornGoldRingImage } from './worn-gold-ring.webp';
// castaway
export { default as CastawayImage } from './castaway.webp';
// silver trinket
export { default as SilverTrinketImage } from './silver-trinket.webp';
// emerald ring
export { default as EmeraldRingImage } from './emerald-ring.webp';
// citrine ring
export { default as CitrineRingImage } from './citrine-ring.webp';
// ruby ring
export { default as RubyRingImage } from './ruby-ring.webp';
// sapphire ring
export { default as SapphireRingImage } from './sapphire-ring.webp';
// opal ring
export { default as OpalRingImage } from './opal-ring.webp';
// signet ring
export { default as SignetRingImage } from './signet-ring.webp';
// opal, pearl, sapphire, emerald, ruby earrings
export { default as OpalEarringsImage } from './opal-earrings.webp';
export { default as PearlEarringsImage } from './pearl-earrings.webp';
export { default as SapphireEarringsImage } from './sapphire-earrings.webp';
export { default as EmeraldEarringsImage } from './emerald-earrings.webp';
export { default as RubyEarringsImage } from './ruby-earrings.webp';
// goblet
export { default as GobletImage } from './goblet.webp';
// fancy boot
export { default as FancyBootImage } from './fancy-boot.webp';
// silver plate
export { default as SilverPlateImage } from './silver-plate.webp';
// repulsion machine
export { default as RepulsionMachineImage } from './repulsion-machine.webp';
// dog
export { default as DogImage } from './dog.webp';
// stone tablets 1,2,3
export { default as StoneTablet1Image } from './stone-tablet-1.webp';
export { default as StoneTablet2Image } from './stone-tablet-2.webp';
export { default as StoneTablet3Image } from './stone-tablet-3.webp';
// pungent, fetid, reeking bait
export { default as PungentBaitImage } from './pungent-bait.webp';
export { default as FetidBaitImage } from './fetid-bait.webp';
export { default as ReekingBaitImage } from './reeking-bait.webp';
// small, large ice block
export { default as SmallIceBlockImage } from './small-ice-block.webp';
export { default as LargeIceBlockImage } from './large-ice-block.webp';
// icebreaker plow half 1,2, icebreaker bracing
export { default as IcebreakerPlowHalf1Image } from './icebreaker-plow-half-1.webp';
export { default as IcebreakerPlowHalf2Image } from './icebreaker-plow-half-2.webp';
export { default as IcebreakerBracingImage } from './icebreaker-bracing.webp';
// ice axe
export { default as IceAxeImage } from './ice-axe.webp';
export { default as FrozenHeartImage } from './frozen-heart.webp';
// ice shaper
export { default as IceShaperImage } from './ice-shaper.webp';
